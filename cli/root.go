// Package cli assembles the alternativeme command tree from the alternativeme
...
// NewApp assembles the kit application from the alternativeme domain. The
...
//
// To add a command, declare it in alternativeme/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := alternativeme.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(alternativeme.Domain{}).Register(app)
