...
describes what users get. Pick a format with `-o`, or let alternativeme choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
alternativeme <command> -o table   # aligned columns for reading
alternativeme <command> -o jsonl   # one JSON object per line, for piping
alternativeme <command> -o json    # a single JSON array
alternativeme <command> -o csv     # spreadsheet friendly
alternativeme <command> -o tsv     # tab-separated
alternativeme <command> -o url     # just the URL column
alternativeme <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
alternativeme <command> --fields id,title,url
```
...
```bash
alternativeme <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
alternativeme <command>            # a table, because this is a terminal
alternativeme <command> | wc -l    # JSONL, because this is a pipe
```
